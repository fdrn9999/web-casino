...
- **이펙트·플레이어 경험 극대화**: 카지노다운 화려함을 1급 요구사항으로 취급한다.
  - 잔액·당첨 금액은 즉시 바뀌지 않고 **카운트업 애니메이션**, 당첨 시 `+N칩` 플로팅 팝업.
  - 카드는 **딜 인(슬라이드) / 플립(딜러 홀카드 공개)** 애니메이션, 칩 베팅 시 칩 애니메이션.
  - 슬롯: 세로 스크롤 릴 연출. 잭팟 당첨은 **풀스크린 연출**(컨페티 + 금액 카운트업 + 화면 흔들림 + mp3).
  - 룰렛: 휠 회전 연출 후 당첨 번호 하이라이트. 블랙잭/바카라: 승리 좌석 글로우, 페이즈 전환 트랜지션.
  - 로비: 게임 카드 호버 이펙트, 잭팟 위젯 반짝임(shimmer), 앰비언트 그라디언트 배경.
  - 모든 연출은 CSS/Canvas 자체 구현(외부 애니메이션 라이브러리 의존 없음), 저사양·모바일에서도 60fps 지향(transform/opacity 위주).
...
## 14. 참고 사이트 반영 기능 (Plan 8, 사용자 승인 2026-07-20)

Evolution/라이브 카지노 사이트(VG Asia, 판도라) UX 참고. **금전 요소(충전/환전/머니이전/페이백)는 제외**, 우리 다크그린+골드 스타일 유지.

1. **라이브 테이블 HUD 강화**: 각 게임 테이블 화면에 하단 잔고/총 베팅액 바, 우상단 컨트롤 클러스터(음소거·히스토리·전체화면), 헤더 베팅 한도, 펠트 위 규칙 문구("블랙잭 3:2" 등), 라운드 번호·"곧 다음 게임" 상태 오버레이.
2. **명예의 전당(리더보드)**: 공개 랭킹 화면 — 최다 보유 칩, 최대 단판 승리(잭팟/블랙잭 등), 총 순손익 상·하위(교육용 '최대 손실' 포함). 서버 집계 API + 로비 위젯/전용 화면.
3. **출석부**: 연속 출석 스트릭 + 단계별 증가 보상(기존 일일 보너스 확장). users에 streak/last_attendance, 서버 검증 + 클라 7일 캘린더.
4. **테이블 라이브 채팅**: 게임 네임스페이스에 table:{id} room 채팅 이벤트 + 간단한 욕설/스팸 필터, 클라 테이블 화면 채팅 패널.
